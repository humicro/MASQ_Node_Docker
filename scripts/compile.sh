...
# Compile MASQNode
cargo $CROSS_MODE --manifest-path node/Cargo.toml --release --verbose
cp node/target/${RUST_CROSS_TARGET}/release/MASQNode ${BUILD_TARGET_PATH}/
# Compile masq
cargo $CROSS_MODE --manifest-path masq/Cargo.toml --release --verbose
cp node/target/${RUST_CROSS_TARGET}/release/masq ${BUILD_TARGET_PATH}/
# Compile dns_utility
cargo $CROSS_MODE --manifest-path dns_utility/Cargo.toml --release --verbose
cp dns_utility/target/${RUST_CROSS_TARGET}/release/dns_utility ${BUILD_TARGET_PATH}/
# Compile port_exposer
cargo $CROSS_MODE --manifest-path port_exposer/Cargo.toml --release --verbose
cp port_exposer/target/${RUST_CROSS_TARGET}/release/port_exposer ${BUILD_TARGET_PATH}/
# Compile automap
cargo $CROSS_MODE --manifest-path automap/Cargo.toml --release --verbose
